...
const getCompilerDataFromContracts = (contracts, flatContracts, hhConfig) => {
    ...
    for (contract of contracts) {
        for (mainContract of flatContracts) {
            ...
            const contractConfig = exports.newCompilerConfig(hhConfig, contract.sourcePath);
            if (config && !exports.compareConfigs(contractConfig, config)) {
...
const getContracts = async (hre, flatContracts) => {
    ...
    const metadata = {
        compiler: {
            version: exports.extractCompilerVersion(hre.config)
        },
        sources: {}
    };
    data._resolvedFiles.forEach((resolvedFile, _) => {
        ...
        for (contract of flatContracts) {
            ...
            const visited = {};
            exports.resolveDependencies(data, sourcePath, metadata, visited);
        }
    });
    for (const [key, value] of Object.entries(metadata.sources)) {
        ...
        const contractToPush = {
            ...
            compiler: {
                name: "solc",
                version: exports.extractCompilerVersion(hre.config, key)
            }
        };
...
const resolveDependencies = (dependencyData, sourcePath, metadata, visited) => {
    ...
    dependencyData._dependenciesPerFile
        ...
        exports.resolveDependencies(dependencyData, resolvedDependency.sourceName, metadata, visited);
