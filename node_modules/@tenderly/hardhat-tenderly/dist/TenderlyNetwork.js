...
class TenderlyNetwork {
    ...
    async getForkContractData(flatContracts) {
        const contracts = await util_1.getContracts(this.env, flatContracts);
        const solcConfig = util_1.getCompilerDataFromContracts(contracts, flatContracts, this.env.config);
        if (solcConfig === undefined) {
            console.log(`Error in ${index_1.PluginName}: No compiler configuration found`);
        }
        return {
            contracts,
            config: solcConfig,
