...
(0, make_suite_1.makeSuite)("General testing of tokens", () => {
    ...
    const amountWETH = ethers_1.ethers.utils.parseEther("1.0");
    // [
    ...
    it("Deposit WETH as collateral and to be borrowed", async () => {
        ...
        expect(signerWeth.toString()).to.be.bignumber.equal(amountWETH, "Did not get WETH");
        ...
        expect(resDat.availableLiquidity.toString()).to.be.bignumber.equal(amountWETH, "Did not deposit WETH");
    });
    it("Testing general tokens deposit and borrow", async () => {
        const tokens = await (0, contracts_helpers_1.getParamPerNetwork)(optimism_1.OptimismConfig.ReserveAssets, types_1.eOptimismNetwork.optimism);
        const config = optimism_1.OptimismConfig.ReservesConfig;
        ...
        for (let [symbol, address] of Object.entries(tokens)) {
            ...
            var USDC = new ethers_1.ethers.Contract(USDCadd, USDCABI);
            ...
            console.log(origAmt);
            await (0, mint_tokens_1.setBalance)(address, signer, origAmt.toString());
            // }
            var signerOrigAmt = await USDC.connect(signer).balanceOf(signer.address);
            //give some to emergency so they can repay debt
            await USDC.connect(signer).approve(emergency.address, ethers_1.ethers.utils.parseEther("100000.0"));
            ...
            expect(signerAmt.toString()).to.not.be.bignumber.equal(0, "Did not get", symbol);
            console.log("Got", signerAmt.toString());
            await USDC.connect(signer).approve(lendingPool.address, ethers_1.ethers.utils.parseEther("100000.0"));
            /************************************************************************************/
            ...
            if (tokenConfig.borrowingEnabled) {
                ...
                const amountBorrowable = (amountWETH.mul(wethPrice).mul(WETHConfig.baseLTVAsCollateral).mul(ethers_1.ethers.utils.parseUnits("1", tokenDec))).div(tokenPrice.mul(ethers_1.ethers.utils.parseUnits("1", WETHdec)).mul(tokenConfig.borrowFactor));
                ...
                const expected = tokenPrice.mul(amountBorrowed).div(ethers_1.ethers.utils.parseUnits("1", tokenDec)); //amount in USD
                console.log("userDat.totalDebtETH.toString(): ", userDat.totalDebtETH.toString());
                console.log("expected: ", expected);
                expect((Math.abs(Number(userDat.totalDebtETH.toString()) - Number(expected)))).to.be.lte(100, "Did not get debt token"); //USD oracles have 8 decimals
                console.log("Passed borrow checks\n\n");
                await expect(lendingPool.connect(emergency).borrow(USDC.address, 0, expectedAmountBorrows, '0', await emergency.getAddress())).to.be.revertedWith(VL_COLLATERAL_CANNOT_COVER_NEW_BORROW);
                console.log("Passed failed borrow checks\n\n");
                console.log("Trying to repay");
                await USDC.connect(emergency).approve(lendingPool.address, ethers_1.ethers.utils.parseEther("100000.0"));
                await lendingPool.connect(emergency).repay(USDCadd, 0, constants_1.MAX_UINT_AMOUNT, await emergency.getAddress());
                console.log("Finished tests for ", symbol);
            }
            else {
                await expect(lendingPool.connect(emergency).borrow(USDCadd, 0, constants_1.MAX_UINT_AMOUNT, '0', await emergency.getAddress())).to.be.revertedWith(VL_BORROWING_NOT_ENABLED);
            }
    ...
    it("wait and harvest all rewards", async () => {
        (0, misc_utils_1.increaseTime)(50000);
        const tokens = await (0, contracts_helpers_1.getParamPerNetwork)(optimism_1.OptimismConfig.ReserveAssets, types_1.eOptimismNetwork.optimism);
        const stakingContracts = await (0, contracts_helpers_1.getParamPerNetwork)(optimism_1.OptimismConfig.ExternalStakingContracts, types_1.eOptimismNetwork.optimism);
        ...
        for (let [symbol, externalRewardsData] of Object.entries(stakingContracts)) {
            ...
            var USDCABI = fs.readFileSync("./localhost_tests/abis/DAI_ABI.json").toString();
            var yvUSDC = new ethers_1.ethers.Contract(USDCadd, USDCABI);
            ...
            let stakingContract;
            if (symbol.slice(-3) != "CRV")
                stakingContract = new ethers_1.ethers.Contract(stakingAddress, Stakingabi);
            else
                stakingContract = new ethers_1.ethers.Contract(stakingAddress, StakingCurveabi);
            const amtStaked = await stakingContract.connect(signer).balanceOf(incentivesController.address);
            let earned;
            if (symbol.slice(-3) != "CRV")
                earned = await stakingContract.connect(signer).earned(incentivesController.address);
            else
                earned = await stakingContract.connect(signer).claimable_reward(incentivesController.address, rewardAdd);
            ...
            // expect(earned).gt(0)
            const rewardToken = new ethers_1.ethers.Contract(rewardAdd, WETHabi);
            const balanceBefore = await rewardToken.connect(signer).balanceOf(incentivesController.address);
            const receipt = await (0, misc_utils_1.waitForTx)(await incentivesController.harvestReward(stakingAddress));
    ...
    it("Testing withdraw everything", async () => {
        ...
        for (let [symbol, address] of Object.entries(tokens)) {
            ...
            if (dat.aTokenAddress == constants_1.ZERO_ADDRESS) {
                continue;
            }
            var userResDat = await dataProv.getUserReserveData(USDCadd, 0, signer.address);
            ...
            /****************** deposit BAL to pool and then borrow WETH  **********************/
            /************************************************************************************/
            const tx = await lendingPool.connect(signer).withdraw(USDCadd, 0, constants_1.MAX_UINT_AMOUNT, signer.address);
            var signerAmt = await USDC.connect(signer).balanceOf(signer.address);
            expect(signerAmt.toString()).to.not.be.bignumber.equal(0, "Did not get back", symbol);
            console.log("Passed withdraw checks\n");
            console.log("-----------------------------------");
        }
    });
});
//# sourceMappingURL=test-general-tokens.spec.js.map