...
describe("Supply - end-to-end test", () => {
    ...
    it("7 - should test that the user has a non-zero amount (ETH) in totalCollateral", async () => {
        let { totalCollateralETH } = await userAggregatedTrancheData({
            signer: owner,
            tranche: 0,
            network: "localhost"
        });
        expect(totalCollateralETH).to.be.bignumber.above(ethers.utils.parseEther("1.0"));
    })
...
describe("Borrow - end-to-end test", () => {
    ...
    it("2 - should swap for USDC with UNISWAP", async () => {
        ...
        expect(await USDC.balanceOf(await temp.getAddress())).to.be.bignumber.above(ethers.utils.parseEther("0"));
    })
    ...
    it("7 - should test borrowing WETH with borrow() fn", async () => {
        ...
        expect(await WETHContract.balanceOf(await temp.getAddress())).to.be.bignumber.above(originalBalance);
    })
