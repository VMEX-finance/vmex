...
makeSuite(
    "testing velodrome oracle against attacks on invariant curve",
    () => {
    const fs = require('fs');
  ...
  const VELO_ROUTER_ABI = fs.readFileSync("./localhost_tests_utils/abis/velo_v2.json").toString()
    ...
    it("test velo pricing", async () => {
       ...
       const testTokens = ["0x0D0F65C63E379263f7CE2713dd012180681D0dc5"]
       for(let i =0;i<testTokens.length;i++){
            const token = testTokens[i]
            const vel = new DRE.ethers.Contract(token, VeloAbi)
