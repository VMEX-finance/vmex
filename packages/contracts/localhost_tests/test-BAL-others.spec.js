...
(0, make_suite_1.makeSuite)("Bal and other tokens", () => {
    ...
    it("give WETH to signer", async () => {
        ...
        var options = { value: misc_utils_1.DRE.ethers.utils.parseEther("1000.0") };
        await myWETH.connect(signer).deposit(options);
        var signerWeth = await myWETH.connect(signer).balanceOf(signer.address);
        expect(signerWeth.toString()).to.be.bignumber.equal(misc_utils_1.DRE.ethers.utils.parseEther("1000.0"), "Did not get WETH");
    });
    it("Deposit WETH for signer to borrow", async () => {
        ...
        expect(resDat.availableLiquidity.toString()).to.be.bignumber.equal(misc_utils_1.DRE.ethers.utils.parseEther("100.0"), "Did not deposit WETH");
    });
    it("Uniswap ETH for BAL and borrow WETH", async () => {
        ...
        expect(signerAmt.toString()).to.not.be.bignumber.equal(0, "Did not get BAL");
        ...
        expect(userResDat.currentATokenBalance.toString()).to.be.bignumber.equal(misc_utils_1.DRE.ethers.utils.parseEther("800"), "Did not get atoken");
        var resDat = await dataProv.getReserveData("0xba100000625a3754423978a60c9317c58a424e3D", 1);
        expect(resDat.availableLiquidity.toString()).to.be.bignumber.equal(misc_utils_1.DRE.ethers.utils.parseEther("800"), "Reserve doesn't have liquidity");
        ...
        expect(userDat.totalDebtETH.toString()).to.be.bignumber.equal(misc_utils_1.DRE.ethers.utils.parseEther("0.1"), "Did not get debt token");
        await expect(lendingPool.connect(signer).borrow(myWETH.address, 1, ethers.utils.parseEther("10"), '0', await signer.getAddress())).to.be.revertedWith(VL_COLLATERAL_CANNOT_COVER_NEW_BORROW);
    });
    it("Uniswap ETH for CRV and borrow WETH", async () => {
        ...
        expect(userResDat.currentATokenBalance.toString()).to.be.bignumber.equal(misc_utils_1.DRE.ethers.utils.parseEther("801"), "Did not get atoken");
        var resDat = await dataProv.getReserveData("0xD533a949740bb3306d119CC777fa900bA034cd52", 1);
        expect(resDat.availableLiquidity.toString()).to.be.bignumber.equal(misc_utils_1.DRE.ethers.utils.parseEther("801"), "Reserve doesn't have liquidity");
        await lendingPool.connect(signer).borrow(myWETH.address, 1, ethers.utils.parseEther("0.1"), '0', await signer.getAddress());
        var userDat = await lendingPool.connect(signer).callStatic.getUserAccountData(signer.address, 1);
        // expect(
    ...
    it("Uniswap ETH for CVX and borrow WETH", async () => {
        ...
        expect(userResDat.currentATokenBalance.toString()).to.be.bignumber.equal(misc_utils_1.DRE.ethers.utils.parseEther(".0001"), "Did not get atoken");
        var resDat = await dataProv.getReserveData("0x4e3FBD56CD56c3e72c1403e103b45Db9da5B9D2B", 1);
        expect(resDat.availableLiquidity.toString()).to.be.bignumber.equal(misc_utils_1.DRE.ethers.utils.parseEther(".0001"), "Reserve doesn't have liquidity");
        await expect(lendingPool.connect(signer).borrow(myWETH.address, 1, ethers.utils.parseEther("10"), '0', await signer.getAddress())).to.be.revertedWith(VL_COLLATERAL_CANNOT_COVER_NEW_BORROW);
    });
    it("Uniswap ETH for BADGER and borrow WETH", async () => {
        ...
        expect(userResDat.currentATokenBalance.toString()).to.be.bignumber.equal(misc_utils_1.DRE.ethers.utils.parseEther("803"), "Did not get atoken");
        var resDat = await dataProv.getReserveData("0x3472A5A71965499acd81997a54BBA8D852C6E53d", 1);
        expect(resDat.availableLiquidity.toString()).to.be.bignumber.equal(misc_utils_1.DRE.ethers.utils.parseEther("803"), "Reserve doesn't have liquidity");
        await lendingPool.connect(signer).borrow(myWETH.address, 1, ethers.utils.parseEther("0.1"), '0', await signer.getAddress());
        var userDat = await lendingPool.connect(signer).callStatic.getUserAccountData(signer.address, 1);
        // expect(
        //     userDat.totalDebtETH.toString()
    ...
    it("Uniswap ETH for LDO and borrow WETH", async () => {
        ...
        expect(userResDat.currentATokenBalance.toString()).to.be.bignumber.equal(misc_utils_1.DRE.ethers.utils.parseEther("804"), "Did not get atoken");
        var resDat = await dataProv.getReserveData("0x5A98FcBEA516Cf06857215779Fd812CA3beF1B32", 1);
        expect(resDat.availableLiquidity.toString()).to.be.bignumber.equal(misc_utils_1.DRE.ethers.utils.parseEther("804"), "Reserve doesn't have liquidity");
        await lendingPool.connect(signer).borrow(myWETH.address, 1, ethers.utils.parseEther("0.1"), '0', await signer.getAddress());
        var userDat = await lendingPool.connect(signer).callStatic.getUserAccountData(signer.address, 1);
        // expect(
        //     userDat.totalDebtETH.toString()
    ...
    it("Uniswap ETH for 1inch and borrow WETH", async () => {
        ...
        expect(userResDat.currentATokenBalance.toString()).to.be.bignumber.equal(misc_utils_1.DRE.ethers.utils.parseEther("805"), "Did not get atoken");
        var resDat = await dataProv.getReserveData("0x111111111117dC0aa78b770fA6A738034120C302", 1);
        expect(resDat.availableLiquidity.toString()).to.be.bignumber.equal(misc_utils_1.DRE.ethers.utils.parseEther("805"), "Reserve doesn't have liquidity");
        await lendingPool.connect(signer).borrow(myWETH.address, 1, ethers.utils.parseEther("0.1"), '0', await signer.getAddress());
        var userDat = await lendingPool.connect(signer).callStatic.getUserAccountData(signer.address, 1);
        // expect(
        //     userDat.totalDebtETH.toString()
    ...
    it("Uniswap ETH for frax and borrow WETH", async () => {
        ...
        /************************************************************************************/
        await lendingPool.connect(signer).deposit(USDC.address, 1, ethers.utils.parseUnits('805'), await signer.getAddress(), '0');
        await expect(lendingPool.connect(signer).setUserUseReserveAsCollateral(USDC.address, 1, true)).to.be.revertedWith(types_1.ProtocolErrors.VL_COLLATERAL_DISABLED);
    });
    it("Uniswap ETH for steth and borrow WETH", async () => {
        ...
        expect(userResDat.currentATokenBalance.toString()).to.be.bignumber.equal(misc_utils_1.DRE.ethers.utils.parseEther("0.01"), "Did not get atoken");
        var resDat = await dataProv.getReserveData("0xae7ab96520DE3A18E5e111B5EaAb095312D7fE84", 1);
        var approx = ethers_1.BigNumber.from(resDat.availableLiquidity.toString()).add(5); //1049...5 to 1050...04 are accepted
        var comp1 = ethers_1.BigNumber.from(approx.toString().slice(0, -1));
        var exp = ethers_1.BigNumber.from(misc_utils_1.DRE.ethers.utils.parseEther("0.01").toString().slice(0, -1));
        console.log(comp1);
        console.log(exp);
...
//# sourceMappingURL=test-BAL-others.spec.js.map