...
(0, make_suite_1.makeSuite)("yearn 3pool ", () => {
    ...
    it("give WETH to users", async () => {
        const lendingPool = await contractGetters.getLendingPool();
        const myWETH = new misc_utils_1.DRE.ethers.Contract(WETHadd, WETHabi);
        var signer = await contractGetters.getFirstSigner();
        //give signer 1 WETH so he can get LP tokens
        var options = { value: misc_utils_1.DRE.ethers.utils.parseEther("1.0") };
        await myWETH.connect(signer).deposit(options);
        var signerWeth = await myWETH.connect(signer).balanceOf(signer.address);
        expect(signerWeth.toString()).to.be.bignumber.equal(misc_utils_1.DRE.ethers.utils.parseEther("1.0"), "Did not get WETH");
        await myWETH.connect(signer).approve(lendingPool.address, misc_utils_1.DRE.ethers.utils.parseEther("100.0"));
    });
    it("deposit WETH", async () => {
        //emergency deposits 100 WETH to pool to provide liquidity
        const myWETH = new misc_utils_1.DRE.ethers.Contract(WETHadd, WETHabi);
        const emergency = (await misc_utils_1.DRE.ethers.getSigners())[1];
        var signer = await contractGetters.getFirstSigner();
        const lendingPool = await contractGetters.getLendingPool();
        const dataProv = await contractGetters.getAaveProtocolDataProvider();
        var options = { value: misc_utils_1.DRE.ethers.utils.parseEther("100.0") };
        await myWETH.connect(emergency).deposit(options);
        var signerWeth = await myWETH.connect(emergency).balanceOf(emergency.address);
        expect(signerWeth.toString()).to.be.bignumber.equal(misc_utils_1.DRE.ethers.utils.parseEther("100.0"), "Did not get WETH");
        await myWETH.connect(emergency).approve(lendingPool.address, misc_utils_1.DRE.ethers.utils.parseEther("100.0"));
        await lendingPool.connect(emergency).deposit(myWETH.address, 1, misc_utils_1.DRE.ethers.utils.parseUnits('100'), await emergency.getAddress(), '0');
        const resDat = await dataProv.getReserveData(myWETH.address, 1);
        expect(resDat.availableLiquidity.toString()).to.be.bignumber.equal(misc_utils_1.DRE.ethers.utils.parseEther("100.0"), "Did not get WETH");
    });
    it("Uniswap ETH for DAI", async () => {
        const DAI = new misc_utils_1.DRE.ethers.Contract(DAIadd, DAI_ABI);
        const myWETH = new misc_utils_1.DRE.ethers.Contract(WETHadd, WETHabi);
        const emergency = (await misc_utils_1.DRE.ethers.getSigners())[1];
        var signer = await contractGetters.getFirstSigner();
        const lendingPool = await contractGetters.getLendingPool();
        const dataProv = await contractGetters.getAaveProtocolDataProvider();
        const UNISWAP_ROUTER_CONTRACT = new ethers_1.ethers.Contract(UNISWAP_ROUTER_ADDRESS, UNISWAP_ROUTER_ABI);
        const path = [myWETH.address, DAI.address];
        const deadline = Math.floor(Date.now() / 1000) + 60 * 20; // 20 minutes from the current Unix time
        //emergency deposits 100 WETH to pool to provide liquidity
        var options = { value: ethers_1.ethers.utils.parseEther("1000.0") };
        await UNISWAP_ROUTER_CONTRACT.connect(signer).swapExactETHForTokens(ethers_1.ethers.utils.parseEther("1000000.0"), path, signer.address, deadline, options);
        var signerDAI = await DAI.connect(signer).balanceOf(signer.address);
        expect(signerDAI.toString()).to.not.be.bignumber.equal(0, "Did not get DAI");
    });
    it("get LP and yv tokens", async () => {
        ...
        const DAI = new misc_utils_1.DRE.ethers.Contract(DAIadd, DAI_ABI);
        const lendingPool = await contractGetters.getLendingPool();
        const yearnVault = new misc_utils_1.DRE.ethers.Contract(yvAddr, yvAbi);
        var triCryptoDeposit = new misc_utils_1.DRE.ethers.Contract(triCryptoDepositAdd, triCryptoDepositAbi);
        var amounts = [ethers_1.ethers.utils.parseEther("10000"), ethers_1.ethers.utils.parseEther("0"), ethers_1.ethers.utils.parseEther("0")];
        await DAI.connect(signer).approve(triCryptoDeposit.address, ethers_1.ethers.utils.parseEther("100000"));
        // await triCryptoDeposit.connect(signer).calc_token_amount([10**2, 10**2,10**2],true)
        await triCryptoDeposit.connect(signer).add_liquidity(amounts, ethers_1.ethers.utils.parseEther("0.1"));
        var CurveToken = new misc_utils_1.DRE.ethers.Contract(CurveTokenAdd, CurveTokenAddabi);
        var mycurve = await CurveToken.connect(signer).balanceOf(signer.address);
        console.log("my curve amount ", mycurve);
        expect(mycurve.toString()).to.not.be.equal("0", "Did not get curve"); //this can be general, we assume that curve's stuff is correct
        await CurveToken.connect(signer).approve(yearnVault.address, ethers_1.ethers.utils.parseEther("100000"));
        ...
        expect(yearnAmount.toString()).to.not.be.equal("0", "Did not get yearn vault tokens"); //this can be general, we assume that curve's stuff is correct
        ...
        // const userCollateralYearn = (await lendingPool.connect(signer).callStatic.getUserAccountData(signer.address,1)).totalCollateralETH.toString();
        ...
        // const userCollateralCurve = (await lendingPool.connect(signer).callStatic.getUserAccountData(signer.address,1)).totalCollateralETH.toString();
    ...
    it("deposit yv and borrow", async () => {
        ...
        var CurveToken = new misc_utils_1.DRE.ethers.Contract(CurveTokenAdd, CurveTokenAddabi);
        const dataProv = await contractGetters.getAaveProtocolDataProvider();
        await yearnVault.connect(signer).approve(lendingPool.address, ethers_1.ethers.utils.parseEther("100000.0"));
        await lendingPool.connect(signer).deposit(yearnVault.address, 1, misc_utils_1.DRE.ethers.utils.parseUnits('1000'), await signer.getAddress(), '0');
        await lendingPool.connect(signer).deposit(yearnVault.address, 1, misc_utils_1.DRE.ethers.utils.parseUnits('500'), await emergency.getAddress(), '0');
        ...
        const curveOracle = new misc_utils_1.DRE.ethers.Contract(curveOracleAdd, curveOracleAbi);
        const pricePerToken = await curveOracle.connect(signer).callStatic.getAssetPrice(yearnVault.address);
        console.log("pricePerToken: ", pricePerToken);
        var col = ethers_2.BigNumber.from(pricePerToken.toString()).mul(1000);
        expect(userDat.totalCollateralETH.toString()).to.be.bignumber.equal(col.toString(), "Did not deposit 3crv");
        await lendingPool.connect(signer).borrow(myWETH.address, 1, misc_utils_1.DRE.ethers.utils.parseEther("0.01"), '0', signer.address);
        userDat = await lendingPool.connect(signer).callStatic.getUserAccountData(signer.address, 1);
        expect(userDat.totalDebtETH.toString()).to.be.bignumber.equal(misc_utils_1.DRE.ethers.utils.parseEther("0.01"), "Did not get debt token");
        var signerWeth = await myWETH.connect(signer).balanceOf(signer.address);
        expect(signerWeth.toString()).to.be.bignumber.equal(misc_utils_1.DRE.ethers.utils.parseEther("1.01"), "Did not get WETH");
        await expect(lendingPool.connect(signer).borrow(myWETH.address, 1, misc_utils_1.DRE.ethers.utils.parseEther("500"), '0', signer.address)).to.be.revertedWith(VL_COLLATERAL_CANNOT_COVER_NEW_BORROW);
    });
    it("deposit more at and withdraw at different timestamps", async () => {
        const lendingPool = await contractGetters.getLendingPool();
        const signer = await contractGetters.getFirstSigner();
        const emergencyAdmin = (await misc_utils_1.DRE.ethers.getSigners())[1];
        const dataProv = await contractGetters.getAaveProtocolDataProvider();
        const yearnVault = new misc_utils_1.DRE.ethers.Contract(yvAddr, yvAbi);
        for (let i = 1; i < 10; i++) {
            // increase time by 1 week
            await misc_utils_1.DRE.ethers.provider.send("evm_increaseTime", [604800]);
            let amount = 100 * i;
            await lendingPool.connect(signer).deposit(yearnVault.address, 1, misc_utils_1.DRE.ethers.utils.parseUnits(amount.toString()), await signer.getAddress(), '0');
    ...
    //       var userData:UserAccountData = await lendingPool.connect(signer).callStatic.getUserAccountData(signer.address,1)
...
//# sourceMappingURL=test-yv3pool.spec.js.map