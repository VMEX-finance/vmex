...
contract BaseOracle {
	...
	constructor(address _pool, uint8 _token_to_price, int decimals) {
        ...
        scale_factor = uint128(decimals >= 0 ?
            WAD / 10 ** uint(decimals) :
            WAD * 10 ** uint(-decimals));		
		
	}	
	
	//returns the latest price in a WAD fixed number 	
	function latest_pool_price() public virtual view returns(uint256) {
