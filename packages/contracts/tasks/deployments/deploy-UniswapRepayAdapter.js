...
(0, config_1.task)(`deploy-${CONTRACT_NAME}`, `Deploys the ${CONTRACT_NAME} contract`)
    ...
    // ];
    const uniswapRepayAdapter = await new types_1.UniswapRepayAdapterFactory(await (0, contracts_getters_1.getFirstSigner)()).deploy(provider, router, weth);
    await uniswapRepayAdapter.deployTransaction.wait();
    console.log(`${CONTRACT_NAME}.address`, uniswapRepayAdapter.address);
    if (verify) {
        await (0, contracts_helpers_1.verifyContract)(types_2.eContractid.UniswapRepayAdapter, uniswapRepayAdapter, [
            provider,
            router,
            weth,
        ]);
    }
    console.log(`\tFinished ${CONTRACT_NAME}${CONTRACT_NAME}lDataProvider proxy and implementation deployment`);
});
//# sourceMappingURL=deploy-UniswapRepayAdapter.js.map