...
import { IAToken } from "../interfaces/IATokens.sol";
...
contract QueryTopTranches {
	constructor(address provider, address dataProvider) {
		...
		uint256[] trancheArray = new uint256[](totalTranches);
		for (uint64 i = 0; i < totalTranches; i++) {
			trancheArray[i] = getTrancheValue(i, dataProvider);
		}
	...
	function getType() public view returns(uint256[] trancheBalances)
	...
	function getUnderlyingToken(AaveProtocolDataProvider.TokenData[] memory token) private view returns(uint256) {
		...
		for (uint i = 0; i < tokens.length; i++) {
			...
			_vtokensp[i] = (tokens[i].tokenAddress);
		}
