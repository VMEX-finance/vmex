...
import { eBaseNetwork, eContractid, eNetwork, IChainlinkInternal, ICommonConfiguration, ProtocolErrors } from '../helpers/types';
import { MAX_UINT_AMOUNT, ZERO_ADDRESS } from "../helpers/constants";
import { getParamPerNetwork } from "../helpers/contracts-helpers";
import {setBalance} from "../localhost_tests_utils/helpers/mint-tokens";
import { ConfigNames, loadPoolConfig } from "../helpers/configuration";
import { getDbEntry } from "../helpers/contracts-getters";
      ...
      const StakingCurveabi = [
        ...
        // ]
          ...
          it("Deposit WETH as collateral and to be borrowed", async () => {
            ...
            var signer = await contractGetters.getFirstSigner();
            const lendingPool = await contractGetters.getLendingPool(await getDbEntry(eContractid.LendingPool, true, network));

            const dataProv = await contractGetters.getAaveProtocolDataProvider(await getDbEntry(eContractid.AaveProtocolDataProvider, true, network));
            ...
            ).to.be.bignumber.gte(amountWETH, "Did not deposit WETH");
          });
          ...
          it("Testing general tokens deposit and borrow", async () => {
            ...
            var dataProv = await contractGetters.getAaveProtocolDataProvider(await getDbEntry(eContractid.AaveProtocolDataProvider, true, network))
            var signer = await contractGetters.getFirstSigner();
            const emergency = (await DRE.ethers.getSigners())[1]
            const myWETH = new DRE.ethers.Contract(WETHadd,WETHabi)
            const addProv = await contractGetters.getLendingPoolAddressesProvider(await getDbEntry(eContractid.LendingPoolAddressesProvider, true, network));
          ...
          it("wait and harvest all rewards", async () => {
            ...
            var signer = await contractGetters.getFirstSigner();
            const incentivesController = await contractGetters.getIncentivesControllerProxy(await getDbEntry(eContractid.IncentivesControllerProxy, true, network));
            const lendingPool = await contractGetters.getLendingPool(await getDbEntry(eContractid.LendingPool, true, network));
