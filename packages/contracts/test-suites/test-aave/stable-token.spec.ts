...
makeSuite("Stable debt token tests", (testEnv: TestEnv) => {
  ...
  it("Tries to invoke mint not being the LendingPool", async () => {
    ...
    const daiStableDebtTokenAddress = (
      await helpersContract.getReserveTokensAddresses(dai.address, 0)
    ).stableDebtTokenAddress;

    const stableDebtContract = await getStableDebtToken(
  ...
  it("Tries to invoke burn not being the LendingPool", async () => {
    ...
    expect(name).to.be.equal("Aave stable debt bearing DAI0");
    await expect(
